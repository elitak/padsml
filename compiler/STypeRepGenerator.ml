...
let rec gen_tyrep dt tc loc = function
  ...
  *)
  ...
  (* we don't deal with the single-variant case *)
  ...
  | SimpleAst.Datatype variants -> gen_datatype_tyrep (gen_specialize dt tc loc) variants
...
let gen dt current_descr tc loc decl = 
  ...
  let ty = SimpleAst.trans dt current_descr tc loc tp_def in
