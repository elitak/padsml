...
%.cmx %.o :$(GEN_DIR)/%.ml
	...
	@mv $(GEN_DIR)/$@ $(BUILD)/$@
	@mv $(GEN_DIR)/$*.o $(BUILD)/$*.o

# compile the .pml to profiled .cmx and .o.
# Use implicit rule to enable both targets to share rule.
# This construction avoids executing the commands twice, once
# for the .cmx and once for the .o.
%_p.cmx  %_p.o :$(GEN_DIR)/%.ml
	$(OCAMLOPT) $(OCAMLFLAGS) -p -c $(GEN_DIR)/$*.ml
	@(if [ -f $(GEN_DIR)/$*.cmi ]; then \
	    mv $(GEN_DIR)/$*.cmi $(BUILD)/$*.cmi;	\
	  fi)
	@mv $(GEN_DIR)/$*.cmx $(BUILD)/$*_p.cmx
	@mv $(GEN_DIR)/$*.o $(BUILD)/$*_p.o
